...
describe('all_config_rules') do
  it 'have a status of COMPLIANT' do
    ...
    # Get Config rules
    rules = client.describe_config_rules(config_rule_names: [],
                                         next_token: '')
    ...
    rules.config_rules.each do |rule|
      comp = client.describe_compliance_by_config_rule(config_rule_names: [rule.config_rule_name],
                                                       compliance_types: [],
                                                       next_token: '')
      ...
      fail_count += 1 if comp_status != 'COMPLIANT'
    end
